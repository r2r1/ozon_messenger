...
class TelegramBotManager:
    ...
    async def _show_settings(self, message_or_query, state: FSMContext):
        ...
        max_orders_text = "∞" if not max_orders else str(max_orders)
        text += f"🧮 Заказы продавца (фильтр): от {min_orders} до {max_orders_text}\n\n"
        
        keyboard = [
            [InlineKeyboardButton(text="🔢 Изменить количество товаров", callback_data="change_default_count")],
            [InlineKeyboardButton(text="🧮 Заказы: ОТ", callback_data="change_min_orders")],
            [InlineKeyboardButton(text="🧮 Заказы: ДО", callback_data="change_max_orders")],
    ...
    async def _change_min_orders(self, query: CallbackQuery, state: FSMContext):
        ...
        await query.message.edit_text("🧮 Введите минимальное количество заказов продавца (0 — отключить фильтр):")
        await query.message.reply("Или нажмите кнопку:", reply_markup=keyboard)
    ...
    async def _change_max_orders(self, query: CallbackQuery, state: FSMContext):
        ...
        await query.message.edit_text("🧮 Введите максимальное количество заказов продавца (0 — без ограничения):")
        await query.message.reply("Или нажмите кнопку:", reply_markup=keyboard)
    ...
    async def _handle_min_orders_input(self, message: Message, state: FSMContext):
        ...
        try:
            ...
            await message.reply("❌ Введите целое число от 0 до 10000000:", reply_markup=keyboard)
            return
        ...
        await message.reply(f"✅ Фильтр по заказам обновлен: минимум {value}", reply_markup=keyboard)
        await state.clear()
    ...
    async def _handle_max_orders_input(self, message: Message, state: FSMContext):
        ...
        max_text = "без ограничения" if value == 0 else f"до {value}"
        await message.reply(f"✅ Фильтр по заказам обновлен: {max_text}", reply_markup=keyboard)
        await state.clear()
